...
The bulk of this file is the [GNOME Extension Best Practices][best-practices]
from gjs.guide, vendored verbatim so it applies to anything editing this
directory. Refresh it from [the upstream Markdown][best-practices-markdown] when
upstream changes.

## This project

`extension/` is the GNOME Shell extension half of Stickies. The Rust application
above it is governed by the repo's CLAUDE.md; these rules apply here only.

The shell process loads four modules and the entry point holds only the
lifecycle: `service.js` is the D-Bus surface, `shortcut.js` the global
keybinding, `taskbarHider.js` the dock hiding. `geometry.js` and `interface.js`
carry no gnome-shell imports so `gjs -m extension/test.js` can exercise them
outside a live session — that suite also enforces several of the rules below
statically, so run it before proposing a change here.

Every Meta method used has been in Mutter since long before the declared
shell-version floor. Do not reintroduce `typeof window.foo === 'function'` or
`?.()` probes around them; the test suite fails on both.
...
[port-guide]: https://gjs.guide/extensions/#upgrading
